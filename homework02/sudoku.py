...
def find_possible_values(grid: List[List[str]], pos: Tuple[int, int]) -> Set[str]:
    ...
    list_block = get_block(grid, pos)
    list_row = get_row(grid, pos)
    list_col = get_col(grid, pos)

    for i in range(9):
        if list_block[i] in values:
            values.remove(list_block[i])
    for i in range(9):
        if list_col[i] in values:
            values.remove(list_col[i])
    for i in range(9):
        if list_row[i] in values:
            values.remove(list_row[i])
